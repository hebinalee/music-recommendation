...
생성된 리포트는 `reports/` 폴더에 `report_YYYYMMDD_HHMMSS_<쿼리슬러그>.html` 형식으로 저장됩니다.
브라우저로 바로 열어 확인하세요.
...
| 파일 | 쿼리 |
|------|------|
| `report_*_브루노마스.html` | "이번 브루노마스 신곡 너무 좋더라 비슷한 좋은 노래 없어?" |
| `report_*_잔잔하고-감성적인-재즈-추천해줘.html` | "잔잔하고 감성적인 재즈 추천해줘" |
| `report_*_BTS-Dynamite-신나는.html` | "BTS Dynamite 같은 신나는 팝 추천" |
...
> **참고**: Spotify Web API는 2024년 11월부터 일반 앱에 `/audio-features` 엔드포인트를 제한합니다.
> 리포트 생성기는 **아티스트 장르 Jaccard 유사도**와 **인기도**를 조합해 추천합니다.
