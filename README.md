...
- **HTML 추천 리포트**: 자연어 입력으로 시각화된 추천 리포트 자동 생성
...
music/
├── scripts/                   # 진입점 스크립트
│   ├── main.py                # 메인 실행 파일 (CLI 인터랙티브 메뉴)
│   ├── generate_report.py     # 자연어 입력 → HTML 추천 리포트 생성
│   ├── example_usage.py       # 사용 예시 및 데모
│   └── run_tests.py           # 테스트 실행
├── reports/                   # 생성된 HTML 추천 리포트 (샘플 포함)
├── README.md                  # 프로젝트 문서
...
사용자 이력이나 플레이리스트 데이터 없이도 **자연어 한 줄**로 추천 리포트를 즉시 생성할 수 있습니다.

```bash
# 자연어 쿼리를 인수로 전달 (기본 모델: multi-signal)
python scripts/generate_report.py "이번 브루노마스 신곡 너무 좋더라 비슷한 거 없어?"

# 한국어·영어 혼용 가능
python scripts/generate_report.py "BTS Dynamite 같은 신나는 팝 추천"

# 추천 개수 지정 (기본값: 20)
python scripts/generate_report.py "잔잔한 재즈 감성" --n 15

# 모델 지정
python scripts/generate_report.py "잔잔한 재즈 감성" --model genre-jaccard
python scripts/generate_report.py "브루노마스 느낌" --model two-tower

# 대화형 모드 (입력 프롬프트 표시)
python scripts/generate_report.py
```

생성된 리포트는 `reports/` 폴더에 `report_YYYYMMDD_HHMMSS_<ModelSlug>_<쿼리슬러그>.html` 형식으로 저장됩니다.
브라우저로 바로 열어 확인하세요.

### 지원 모델 (`--model`)

| `--model` 값 | 모델명 | 파일명 슬러그 | 설명 |
|---|---|---|---|
| `multi-signal` **(기본값)** | Multi-Signal Genre Ranker | `MultiSignal` | 씨앗 기반 · Genre Jaccard 50% + Popularity 30% + Recency 10% − Artist Penalty 40% |
| `genre-jaccard` | Genre-Jaccard Ranker | `GenreJaccard` | 레거시 · Genre Jaccard 60% + Popularity 40% (아티스트 다양성 처리 없음) |
| `two-tower` | Two-Tower Item Ranker | `TwoTower` | 아이템 임베딩 + FAISS ANN · **사전 학습 필요** (`scripts/main.py` 메뉴 6·7) |

> **Two-Tower 사용 전 학습 필요**: `models/two_tower_model.pth`, `models/faiss_index.bin`, `models/faiss_id_map.json` 파일이 없으면 자동으로 Multi-Signal로 폴백합니다.

> **Spotify API 제한**: 2024년 11월부터 `/audio-features`가 일반 앱에 차단됩니다. Two-Tower 모델은 장르 태그로부터 오디오 피처를 추정하는 Proxy Feature 방식을 사용합니다.

### reports/ 샘플 리포트

`reports/` 폴더에 아래와 같은 샘플 리포트가 포함되어 있습니다.
Spotify API 키 없이도 리포트의 레이아웃과 내용을 미리 확인할 수 있습니다.

| 파일 | 쿼리 | 모델 |
|------|------|------|
| `report_*_MultiSignal_브루노마스.html` | "이번 브루노마스 신곡 너무 좋더라 비슷한 좋은 노래 없어?" | multi-signal |
| `report_*_MultiSignal_잔잔하고-감성적인-재즈-추천해줘.html` | "잔잔하고 감성적인 재즈 추천해줘" | multi-signal |
| `report_*_MultiSignal_BTS-Dynamite.html` | "BTS Dynamite 같은 신나는 팝 추천" | multi-signal |
| `report_*_GenreJaccard_*.html` | (이전 버전 리포트) | genre-jaccard |

### 리포트 구성

- **모델명**: 헤더에 사용된 모델명 표시
- **씨앗 트랙(Seed Tracks)**: 쿼리에서 찾은 기준 곡 (인기도, 장르, Spotify 링크)
- **장르 분포 차트**: 추천 트랙의 장르 분포 도넛 차트 (Chart.js)
- **추천 트랙**: 유사도 점수 순으로 정렬된 추천 목록 (미리듣기 링크 포함)
